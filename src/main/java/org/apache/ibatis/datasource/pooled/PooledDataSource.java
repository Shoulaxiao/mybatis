...
public class PooledDataSource implements DataSource {
    ...
    synchronized (state) {
      ...
      if (conn.isValid()) {
        ...
        if (state.idleConnections.size() < poolMaximumIdleConnections && conn.getConnectionTypeCode() == expectedConnectionTypeCode) {
          // 累积增加accumulatedCheckoutTime
          state.accumulatedCheckoutTime += conn.getCheckoutTime();
          if (!conn.getRealConnection().getAutoCommit()) {
            // 回滚未提交的事务
            conn.getRealConnection().rollback();
          }
          // 将底层连接重新封装成PooledConnection对象,并且添加到空闲集合中
          PooledConnection newConn = new PooledConnection(conn.getRealConnection(), this);
          state.idleConnections.add(newConn);
          // 设置创建时间戳和最后使用时间戳
          newConn.setCreatedTimestamp(conn.getCreatedTimestamp());
          newConn.setLastUsedTimestamp(conn.getLastUsedTimestamp());
          // 丢弃旧PooledConnection对象
          conn.invalidate();
          if (log.isDebugEnabled()) {
        ...
        // 当前PooledDataSource连接池中的空闲连接已经达到上限值，当前数据库连接无法放回到池中
        else {
          //累积accumulatedCheckoutTime
          state.accumulatedCheckoutTime += conn.getCheckoutTime();
          if (!conn.getRealConnection().getAutoCommit()) {
            // 回滚未提交的事务
            conn.getRealConnection().rollback();
      ...
      synchronized (state) {
        ...
        // 没有空闲连接
          ...
          if (state.activeConnections.size() < poolMaximumActiveConnections) {
            // 创建数据库连接，并且封装为PooledConnection对象
  ...
  protected boolean pingConnection(PooledConnection conn) {
    ...
    // 如果底层数据库连接网络没有断开，则需要检测poolPingEnabled字段的配置，决定是否能执行ping操作。
    // ping操作不能频繁执行，只有超时一定时长未使用的连接，才需要ping
